...
class GoRight(gym.Env):
    ...
    def __init__(self):
        ...
        # 初期位置の指定
        self.agent_pos = self.ZERO
        # 行動空間と状態空間の定義
        self.action_space = gym.spaces.Discrete(6)
        self.observation_space = gym.spaces.Box(low=0, high=self.GRID_SIZE, shape=(1,), dtype=np.float32)
    ...
    def step(self, action):
        # 移動
        if action == self.agent_pos+1:
            self.agent_pos += 1
#         else:
#             con
#         self.agent_pos = np.clip(self.agent_pos, 0, self.GRID_SIZE)
        # エピソード完了の計算
        done = self.agent_pos == self.GRID_SIZE
        # 報酬の計算
